...
def determine_course_value(grade, course, courses_dict):
    ...
    # NEW: Parse the passing grades list for this course
    ...
    if pd.isna(grade):
        ...
        # Split the grade string (supporting multiple entries)
        grades = grade.split(', ')
        grades_cleaned = [g.strip().upper() for g in grades if g.strip()]
        all_grades = ', '.join(grades_cleaned)
        # Check if any of the student's grades is one of the allowed passing grades

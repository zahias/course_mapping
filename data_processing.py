...
def determine_course_value(grade, course, courses_dict, grading_system):
    ...
    - If the grade is NaN, return 'NR'.
    - If the grade is an empty string (''), it is interpreted as the student being currently registered.
      In that case, return "CR | <credits>".
    - Otherwise, split the comma-separated grade string.
      Then, using the provided grade ranking, determine the best (highest ranked) grade among those
      that are considered counted (i.e. present in grading_system['Counted']).
      If a best grade exists and its ranking is at least that of the course’s passing threshold (default threshold is 'D-'),
      return the full comma-separated grade string with " | <credits>" appended.
      If not, return the full comma-separated grade string with " | 0".
    """
    # Define ranking (higher is better)
    grade_ranking = {
        'A+': 14, 'A': 13, 'A-': 12,
        'B+': 11, 'B': 10, 'B-': 9,
        'C+': 8, 'C': 7, 'C-': 6,
    ...
    # Get the passing threshold for the course (from configuration)
    thresholds = get_course_passing_thresholds()  # Assume this function exists in config.py

...
def process_progress_report(df, target_courses, intensive_courses, per_student_assignments=None, equivalent_courses_mapping=None):
    ...
    # Pivot for required courses.
    pivot_df = df.pivot_table(index=['ID','NAME'], columns='Mapped Course', values='Grade', 
                              aggfunc=lambda x: ', '.join(x.dropna().astype(str))).reset_index()
    ...
    # Extra courses for assignment editing remain as-is.
...
def determine_course_value(grade, course, courses_dict):
    """
    Returns a CourseResult for a given course based on the student's grade string.
    The course configuration (from courses_dict) includes:
      - Credits (an int)
      - PassingGrades (a comma-separated string of allowed passing grades)
    The returned CourseResult has:
      - display: a string in the format: "{grade tokens} | {credit or marker}"
      - passed: True if at least one grade token is in the allowed list; False otherwise;
                or None if currently registered (CR)
      - credit: the course's credit if passed (for >0 courses), or 0 if failed.
                For 0-credit courses, the marker is "PASS" or "FAIL" but credit remains 0.
    """

...
from config import GRADE_ORDER, get_allowed_assignment_types
...
def transform_wide_format(df):
    ...
    if split_cols.shape[1] < 3:
        st.error("Wide format parsing failed; expected format: COURSECODE/SEMESTER-YEAR/GRADE.")
        return None
    ...
    if sem_year_split.shape[1] < 2:
        st.error("Unrecognized Semester-Year format.")
        return None
...
def determine_course_value(grade, course, courses_dict):
    ...
    # Instead of a single passing threshold, we expect a comma-separated list
    passing_grades = [x.strip().upper() for x in info["PassingGrade"].split(',')]
    if pd.isna(grade):
        return 'NR'
    elif grade == '':
        return f'CR | {credits}'
    ...
    for course, info in courses_dict.items():
        ...
        if isinstance(value, str):
            ...
            if value_upper.startswith('CR'):
                ...
                passing_grades = [x.strip().upper() for x in info["PassingGrade"].split(',')]
                passing = any(g in passing_grades for g in grades_list)
                if passing:
                    completed += credit
                else:
                    remaining += credit
    ...
    for r_idx, row in enumerate(dataframe_to_rows(displayed_df, index=False, header=True), 1):
        for c_idx, value in enumerate(row, 1):
            ...
            if r_idx == 1:
                ...
                if isinstance(value, str) and value.upper().startswith('CR'):

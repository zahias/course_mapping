...
with st.expander("Course Configuration Options", expanded=True):
    ...
    if uploaded is not None:
        try:
            ...
            # Detect if PassingGrades got split across multiple columns
            cols = list(courses_df.columns)
            if len(cols) > 5 or 'PassingGrades' not in cols:
                # Expect columns: Course, Credits, PassingGrades, Type, EffectiveSemester
                # Merge cols[2:-2] into PassingGrades, then grab last two as Type & EffectiveSemester
                pg_cols = cols[2:-2]
                type_col = cols[-2]
                eff_col = cols[-1]
                # Reconstruct PassingGrades
                courses_df['PassingGrades'] = courses_df[pg_cols].astype(str).agg(','.join, axis=1)
                courses_df['Type'] = courses_df[type_col]
                courses_df['EffectiveSemester'] = courses_df[eff_col]
                # Drop the now-merged columns
                courses_df = courses_df.drop(columns=pg_cols + [type_col, eff_col])

...
st.write(
    ...
    **Optional** columns:\n\n"
    ...
    "If you omit Effective dates, the rule applies for all time."
)
...
with st.expander("Course Configuration Options", expanded=True):
    ...
    with col1:
        if st.button("Download Template"):
            tmpl = pd.DataFrame({
                ...
                'Effective_From': ['SPRING-2023','SPRING-2023'],
                'Effective_To': ['FALL-2023','']
            })
            st.download_button(
        ...
        if st.button("Reload from Google Drive"):
            try:
                ...
                if fid:
                    ...
                    st.success("courses_config.csv reloaded locally.")
                else:
    ...
    if df is not None:
        ...
        if not req.issubset(df.columns):
            st.error(f"Missing columns: {req - set(df.columns)}")
        else:
            # Helper to parse Effective_* into tuples or None
            def parse_eff(val):
                if pd.isna(val) or not str(val).strip():
                    return None
                s,y = val.split('-',1)
                return (s.strip().title(), int(y))
            # Build dicts of lists
            target_cfg = {}
            intensive_cfg = {}
            ...
            st.success("Course configurations loaded into session.")
    else:

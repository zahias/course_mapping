...
if 'raw_df' not in st.session_state:
    ...
    # Count each unique student only once.
    unique_students = df.drop_duplicates(subset=["ID"])
    unique_students["Cohort"] = unique_students["ID"].astype(str).str[:4]
    cohort_df = unique_students.groupby("Cohort").size().reset_index(name="Student Count")
    ...
    st.header("Average Grade Frequency")

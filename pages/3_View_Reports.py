...
# Simplified primary-grade (with credits)
...
show_c = st.checkbox("Show Completed/Not Completed Only", False)
if show_c:
    def collapse(v):
        if not isinstance(v,str): return v
        p=v.split("|")
        if len(p)==2:
            try: return "c" if int(p[1].strip())>0 else ""
            except: return "c" if p[1].strip().upper()=="PASS" else ""
        return v
    for col in target: disp_req[col]=disp_req[col].apply(collapse)
    for col in intensive: disp_int[col]=disp_int[col].apply(collapse)

...
if "raw_df" not in st.session_state:
    ...
    if target_courses is None or intensive_courses is None:
        ...
        completed_toggle = st.checkbox("Show Completed/Not Completed Only", value=False, help="If checked, shows 'c' if completed and '' if not instead of actual grades.")
        
        def extract_primary_grade(value):
            if isinstance(value, str) and "|" in value:
                parts = value.split("|")
                grades_part = parts[0]
                credits_part = parts[1].strip()
                grades_list = [g.strip() for g in grades_part.split(",") if g.strip()]
                if grade_toggle:
                    return ", ".join(grades_list)
                else:
                    # Only display 'c' if the numeric credit is not 0 and value does not start with CR.
        ...
        st.markdown("- Red: Grades that do not count (failing based on the course's passing list)")
        st.markdown("- Pink: Fallback (e.g. missing data)")
